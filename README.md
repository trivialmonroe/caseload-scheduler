...
3. Delete the default `Code.gs` file's contents entirely, then paste in **Constants.gs** from this project.
...
- **`Printable_Schedule`**: a clean, shareable calendar view — colored blocks sized and positioned by each session's actual time, with the time range and teacher shown under each name. Build/refresh it any time via **Caseload Scheduler → Build Printable Schedule (2-Week View)** (it reads straight from `Schedule_Log`, so no need to re-run the full scheduler just to rebuild this view). Each grade gets a consistent, vibrant color with a legend at the top; group sessions collapse into one block showing every member's name. If you have any Quarterly students, only the first 2-week pair is shown (Week 1 + 2) — with **Prefer Consistent Weekly Pattern** on, later weeks normally look the same anyway; check `Schedule_Log` if you need to verify a specific later week.
